...
double calcBSPrice(const OptionParameters& option) {
    ...
    double y1 = (std::log(option.spotPrice_ / option.strikePrice_) + option.timeToMaturity_
                * (option.riskFreeRate_ + option.volatility_ * option.volatility_  * 0.5))
                / volSqrtTime;
    ...
    if (option.optionType_ == finutils::OptionType::Call) {
        price = option.spotPrice_ * calcCDF(y1) - option.strikePrice_
                * std::exp(-option.riskFreeRate_ * option.timeToMaturity_) * calcCDF(y2);
    }
    else {
